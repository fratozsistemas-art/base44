...
export default function ConversationList({
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      console.log('🗑️ Starting delete process for conversation:', conversationToDelete.id);
      
      // Call backend function with proper error handling
      const response = await base44.functions.invoke('deleteConversation', {
      ...
      if (response.data?.success) {
        ...
        const errorMsg = response.data?.error || 'Erro ao excluir conversa';
      ...
      // Parse error message
      let errorMessage = 'Erro ao excluir conversa';
      ...
      if (errorMessage.includes('permission') || errorMessage.includes('Unauthorized')) {
        toast.error('❌ Você não tem permissão para deletar esta conversa');
      ...
      } else if (errorMessage.includes('own conversations')) {
        toast.error('❌ Você só pode deletar suas próprias conversas');
      } else {
  ...
  const renderConversation = (conversation) => {
    ...
    return (
      ...
      >
        ...
        {/* Delete Button - Always visible on hover or when active */}
        <Button
          variant="ghost"
          size="icon"
